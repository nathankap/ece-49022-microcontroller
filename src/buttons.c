...
#include "delay.h"
#include "spi.h"
#include "motors.h"  // Include the motors header file
#include <string.h>
        ...
        case 0:  // Mode 1: Random horizontal and vertical movements while shooting
            if (is_button_pressed(GPIO_IDR_4)) {  // Only toggle while shooting
                if (get_tick() - last_toggle_time > 200) {  // Toggle every 200ms
                    // Toggle vertical motor (PB6 equivalent)
                    static motor_direction_t vertical_direction = MOTOR_CW;
                    vertical_direction = (vertical_direction == MOTOR_CW) ? MOTOR_CCW : MOTOR_CW;
                    set_motor1_direction(vertical_direction);

                    // Toggle horizontal motor (PB7 equivalent)
                    static motor_direction_t horizontal_direction = MOTOR_CW;
                    horizontal_direction = (horizontal_direction == MOTOR_CW) ? MOTOR_CCW : MOTOR_CW;
                    set_motor2_direction(horizontal_direction);

                    last_toggle_time = get_tick();
                }
            } else {
                // Stop both motors when not shooting
